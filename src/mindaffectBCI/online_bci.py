...
def startacquisitionProcess(acquisition, acq_args, label='online_bci', logdir=None):
    ...
    Args:
        ...
        acquisition (str): the name for the acquisition device to start.  One-of:
                  'none' - do nothing,  
                  'brainflow' - use the mindaffectBCI.acquisition.utopia_brainflow driver
    ...
    if acquisition.lower() == 'none':
        ...
        from mindaffectBCI.acquisition import utopia_brainflow
        if acq_args is None:
        ...
        from mindaffectBCI.acquisition import utopia_ganglion
        ...
        from mindaffectBCI.acquisition import utopia_cyton
...
def startPresentationProcess(presentation,presentation_args):
    ...
    if presentation.lower() == 'selectionMatrix'.lower() or presentation.lower() == 'mindaffectBCI.presentation.selectionMatrix'.lower():
        ...
        from mindaffectBCI.presentation import colorwheel
        target=colorwheel.run

    elif presentation.lower() == 'rpigpio':
        from mindaffectBCI.presentation import rpigpio
        target = rpigpio.run
...
def load_config(config_file):
    ...
    if isinstance(config_file,str):
        ...
        config_file = search_directories_for_file(config_file, os.path.join(os.path.dirname(os.path.abspath(__file__)), 'configs'))
